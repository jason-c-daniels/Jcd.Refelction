#region

using System;
using System.Collections;
using System.Linq;
using System.Reflection;

using Jcd.Reflection.Tests.Fakes;

using Xunit;

// ReSharper disable HeapView.DelegateAllocation
// ReSharper disable HeapView.ObjectAllocation
// ReSharper disable HeapView.ClosureAllocation
// ReSharper disable InconsistentNaming
// ReSharper disable HeapView.BoxingAllocation
// ReSharper disable HeapView.ObjectAllocation.Evident

#endregion

namespace Jcd.Reflection.Tests;

public class MemberInfoEnumeratorTests
{
   /// <summary>
   /// Validate that Enumerate Enumerates AllPublicFieldsAndProperties When GivenAnObjectWithFieldsAndProperties.
   /// </summary>
   [Fact]
   public void Enumerate_WhenGivenAnObjectWithFieldsAndProperties_EnumeratesAllPublicFieldsAndProperties()
   {
      var obj  = new TestClassB();
      var sut  = new MemberInfoEnumerator(obj);
      var list = sut.ToList();
      Assert.Equal(17, list.Count);
   }

   /// <summary>
   /// Validate that Enumerate Enumerates AllPublicFieldsAndProperties When GivenAnObjectWithFieldsAndProperties.
   /// </summary>
   [Fact]
   public void
      Enumerate_WhenGivenAnObjectWithFieldsAndPropertiesAndBingingFlags_EnumeratesAllSpecifiedFieldsAndProperties()
   {
      var obj = new TestClassB();
      var sut = new MemberInfoEnumerator(obj
                                       , new MemberInfoFilter
                                         {
                                            Flags = BindingFlags.Instance
                                                  | BindingFlags.Static
                                                  | BindingFlags.NonPublic
                                                  | BindingFlags.Public
                                         }
                                        );
      var list = sut.ToList();
      Assert.Equal(35, list.Count);
      sut  = new MemberInfoEnumerator(obj, MemberInfoFilter.AllInstanceMembersFilter);
      list = sut.ToList();
      Assert.Equal(24, list.Count);
      sut  = new MemberInfoEnumerator(obj, MemberInfoFilter.AllStaticMembersFilter);
      list = sut.ToList();
      Assert.Equal(17, list.Count);
      sut  = new MemberInfoEnumerator(obj, MemberInfoFilter.DirectInstanceMethodsFilter);
      list = sut.ToList();
      Assert.Equal(6, list.Count);
      sut  = new MemberInfoEnumerator(obj, MemberInfoFilter.DirectStaticMembersFilter);
      list = sut.ToList();
      Assert.Equal(11, list.Count);

      // force coverage on IEnumerable.GetEnumerator
      var       disp    = ((IEnumerable) sut).GetEnumerator();
      using var unknown = disp as IDisposable;
   }
}